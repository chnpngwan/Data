...
public class MainFrame extends JFrame {
	...
	public MainFrame() throws SQLException {
		...
		lblNewLabel.setBounds(37, 29, 85, 37);
		contentPane.add(lblNewLabel);
		
		textField = new JTextField();
		textField.setBounds(146, 29, 334, 39);
		...
		btnNewButton.setBounds(570, 42, 93, 23);
		...
		button.setBounds(688, 43, 93, 23);
		contentPane.add(button);
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(26, 100, 794, 377);
		contentPane.add(scrollPane);
		
		Dao dao = new Dao();
		List<Persional> list=dao.select(null);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			data[i][7]=list.get(i).getRegister_date();
		}
		table = new JTable();
	...
	protected void selectInfo(ActionEvent e) throws SQLException {
		...
		List<Persional> list=new Dao().select(city);
		Object[][] data=new Object[list.size()][8];
		for (int i = 0; i < list.size(); i++) {
			data[i][0]=list.get(i).getId();
			data[i][1]=list.get(i).getCity();
